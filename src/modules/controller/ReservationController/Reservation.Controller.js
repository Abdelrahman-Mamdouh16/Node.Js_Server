...
export const createCheckoutSessionOnTelehealth = async (req, res, next) => {
    try {
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            mode: 'payment',
            success_url: `https://online-doc-app.vercel.app/reservationOnTelehealth/createCall`,
            cancel_url: `https://online-doc-app.vercel.app/`,
            customer_email: user.email,
            client_reference_id: user._id
        });
        if (!session)
            res.json({ success: false, message: 'The Checkout Session not created' });
        else {
            res.json({ success: true, message: 'The Checkout Session is created', result: session });
        }
    } catch (error) {
        console.error(error);
        return res.json({ success: false, message: `Internal server error :${error}` });
